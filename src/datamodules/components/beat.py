...
class CustomDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 pose_fps = 15,
                 pose_dims = 141,
                 speaker_dims = 30,  # args.speaker_dims
                 audio_rep = "wave16k",
                 pose_rep = "bvh_rot",  # args.pose_rep
                 facial_rep = "facial52",  # args.facial_rep
                 word_rep = "text",  # args.word_rep
                 emo_rep = "emo",  # args.emo_rep
                 sem_rep = "sem",  # args.sem_rep
                 audio_fps = 16000,  # args.audio_fps
                 id_rep = "id",  # args.speaker_id

                 disable_filtering = False,  # args.disable_filtering
                 clean_first_seconds = 0,  # args.clean_first_seconds
                 clean_final_seconds = 0,  # args.clean_final_seconds
                 audio_norm = False,
                 facial_norm = False,
                 augmentation=None, 
                 kwargs=None, 
                 build_cache=False):
        log.info("CustomDataset is initialing")
        self.loader_type = loader_type
        self.rank = 0 #dist.get_rank()
        self.new_cache = new_cache #False #args.new_cache
        self.pose_length = pose_length #34 #args.pose_length  # 34
        self.stride = stride #10 # args.stride  # 10
        self.pose_fps = pose_fps #15 # args.pose_fps  # 15
        self.pose_dims = pose_dims #141 #args.pose_dims  # 141

        self.speaker_dims = speaker_dims # args.speaker_dims
        self.loader_type = loader_type
        self.audio_rep = audio_rep#args.audio_rep
        self.pose_rep = pose_rep #args.pose_rep
        self.facial_rep = facial_rep #args.facial_rep
        self.word_rep = word_rep #args.word_rep
        self.emo_rep = emo_rep #args.emo_rep
        self.sem_rep = sem_rep #args.sem_rep
        self.audio_fps = audio_fps #args.audio_fps
        self.id_rep = id_rep #args.speaker_id

        self.audio_norm = audio_norm
        self.facial_norm = facial_norm
        ...
        self.clean_final_seconds = clean_final_seconds #args.clean_final_seconds
        ...
        if loader_type == "train":
            self.data_dir = self.root_path + self.train_data_path
            self.multi_length_training = self.multi_length_training #! note: the float
        elif loader_type == "val":
            self.data_dir = self.root_path + self.val_data_path
            self.multi_length_training = self.multi_length_training
        else:
            self.data_dir = self.root_path + self.test_data_path
            self.multi_length_training = [1.0]

        self.max_length = int(self.pose_length * self.multi_length_training[-1])

        if self.word_rep is not None:
            with open(f"{self.root_path}{self.train_data_path[:-6]}vocab.pkl", 'rb') as f:
                self.lang_model = pickle.load(f)
        preloaded_dir = self.data_dir + f"{self.pose_rep}_cache"

        self.mean_pose = np.load(self.root_path + self.mean_pose_path + f"{self.pose_rep}/bvh_mean.npy")
        self.std_pose = np.load(self.root_path + self.mean_pose_path + f"{self.pose_rep}/bvh_std.npy")

        if self.audio_norm:
            self.mean_audio = np.load(self.root_path + self.mean_pose_path + f"{self.audio_rep}/npy_mean.npy")
            self.std_audio = np.load(self.root_path + self.mean_pose_path + f"{self.audio_rep}/npy_std.npy")
        if self.facial_norm:
            self.mean_facial = np.load(self.root_path + self.mean_pose_path + f"{self.facial_rep}/json_mean.npy")
            self.std_facial = np.load(self.root_path + self.mean_pose_path + f"{self.facial_rep}/json_std.npy")
        ...
        for pose_file in pose_files:
            ...
            if self.facial_rep is not None:
                ...
                try:
                    ...
                    logger.warning(
                        f"# ---- file not found for Facial {id_pose}, skip all files with the same id ---- #")
                    continue
            ...
            if self.emo_rep is not None:
                ...
                for i in range(pose_each_file.shape[0]):
                    ...
                    for j, (start, end, score) in enumerate(
                            zip(emo_all['start_time'], emo_all['end_time'], emo_all['score'])):
                        current_time = i / self.pose_fps + time_offset
            ...
            if self.sem_rep is not None:
                ...
                for i in range(pose_each_file.shape[0]):
                    found_flag = False
                    for j, (start, end, score) in enumerate(
                            zip(sem_all['start_time'], sem_all['end_time'], sem_all['score'])):
                        current_time = i / self.pose_fps + time_offset
                        if start <= current_time and current_time <= end:
    ...
    def _sample_from_clip(
        ...
        if audio_each_file != []:
            ...
            if facial_each_file != []:
                ...
                logger.info(
                    f"audio: {round_seconds_skeleton}s, pose: {round_seconds_audio}s, facial: {round_seconds_facial}s")
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            try:
                ...
                key = "{:005}".format(3998).encode("ascii")
                print("出错了 idx, key", idx, key)
                sample = txn.get(key)
            ...
            return {"x": tar_pose, "cond": in_audio}
